...
export class DeliveryController {
    ...
    schema: {
      ...
      properties: {
        ...
        deliveryPhoto: {
          type: 'string',
          format: 'binary',
          description: 'Foto del asesor con el vehículo',
        },
        signedActa: {
          type: 'string',
          format: 'binary',
          description: 'Foto del acta firmada por el cliente',
        },
      },
    },
  })
  @ApiResponse({
    status: 201,
    description:
      'Entrega completada. Retorna vehicleId, newStatus: ENTREGADO, deliveryDate',
  })
  @ApiResponse({
    status: 400,
    description: 'Vehículo no está AGENDADO o no es el día de entrega',
  })
  @ApiResponse({
    status: 403,
    description: 'No eres el asesor asignado al agendamiento',
  })
  @ApiResponse({
    status: 404,
    description: 'Vehículo o agendamiento no encontrado',
  })
  @Roles(
    RoleEnum.ASESOR,
  ...
  @ApiResponse({
    status: 200,
    description: 'Datos de la ceremonia con deliveryPhotoUrl y signedActaUrl',
  })
  @ApiResponse({
    status: 404,
    description: 'Ceremonia de entrega no encontrada',
  })
  @Roles(
    RoleEnum.ASESOR,
    RoleEnum.LIDER_TECNICO,
