...
/**
 * DocumentationRepository no agrega lógica propia — es una instanciación de
 * TenantScopedRepository para `documentations`. Estos tests cubren el
 ...
 * el comportamiento que DocumentationService asume que existe.
 */
describe('DocumentationRepository', () => {
  ...
  beforeEach(() => {
    ...
    repository = new DocumentationRepository(firebase as never, audit as never);
  });
  ...
  describe('aislamiento entre concesionarios', () => {
    it('findById() devuelve null ante una documentación de otro concesionario', async () => {
    ...
    it('findById() devuelve la documentación propia', async () => {
    ...
    it('findByIdOrThrow() lanza el NotFound del llamador ante un documento ajeno (nunca 403)', async () => {
      givenDocument({ tenantId: 'mazda-guayaquil', vehicleId: 'vehicle-1' });

      await expect(
        TenantContext.run(makeContext(), () =>
          repository.findByIdOrThrow('vehicle-1', () => new Error('404')),
        ),
      ).rejects.toThrow('404');
    });
  });
